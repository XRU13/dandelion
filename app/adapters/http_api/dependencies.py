...
async def get_user_service(
    ...
    user_score_repository: UserScoreRepository = Depends(get_user_score_repository)
) -> UserService:
...
async def get_event_service(
    ...
    return EventService(event_repository, user_score_repository, user_repository, redis_repository)


async def get_stats_service(
    user_repository: UserRepository = Depends(get_user_repository),
    user_score_repository: UserScoreRepository = Depends(get_user_score_repository),
    user_achievement_repository: UserAchievementRepository = Depends(get_user_achievement_repository),
    redis_repository: RedisUserScoreRepository = Depends(get_redis_repository)
) -> StatsService:
    """Фабрика для создания сервиса статистики"""
    return StatsService(user_repository, user_score_repository, user_achievement_repository, redis_repository)


async def get_achievement_service(
    achievement_repository: AchievementRepository = Depends(get_achievement_repository),
    user_achievement_repository: UserAchievementRepository = Depends(get_user_achievement_repository),
    achievement_notification_repository: AchievementNotificationRepository = Depends(get_achievement_notification_repository),
    user_score_repository: UserScoreRepository = Depends(get_user_score_repository),
    redis_repository: RedisUserScoreRepository = Depends(get_redis_repository)
) -> AchievementService:
